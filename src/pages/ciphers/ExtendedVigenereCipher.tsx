...
export default function ExtendedVigenereCipher() {
  ...
  const keyExpand = React.useCallback(
    (cipherkey: string, messagelength: number) => {
      ...
      // }
      console.log("extendedVigenere", expandTimes);
      return newKey;
    },
    []
  );

  const extendedVigenereCipher = React.useCallback(
    (plaintext: string, cipherkey: string, encrypt: boolean) => {
      let text = "";
      let currText = plaintext.split(" ").join("");
      cipherkey = keyExpand(cipherkey, currText.length);
      for (let i = 0; i < currText.length; i++) {
        if (encrypt) {
          let coded = currText.charCodeAt(i) + cipherkey.charCodeAt(i);
          if (coded > 255) coded -= 255;
          text += String.fromCharCode(coded);
          console.log("extendedVigenere en", currText, text, cipherkey);
        } else {
          let coded = currText.charCodeAt(i) - cipherkey.charCodeAt(i);
          if (coded < 0) coded += 255;
          text += String.fromCharCode(coded);
          console.log("extendedVigenere dec", currText, text, cipherkey);
        }
      }
      return text;
    },
          ...
          >
            ...
            {inputMode === "1" && (
              <Input
                ...
                maxH="100%"
                overflow="auto"
                aria-label="extendedVigenere-input"
            ...
            {inputMode === "2" && (
              <Box display="flex" w="100%" flexDirection="column">
                <Input
                  ...
                  w="90%"
                  overflow="auto"
                  aria-label="extendedVigenere-file-input"
                  verticalAlign="center"
                  alignSelf="center"
                  textAlign="center"
                ></Input>
            ...
            <Input
              ...
              maxH="100%"
              overflow="auto"
              aria-label="extendedVigenere-key-input"
              alignSelf="center"
            ></Input>
            ...
            <Stack direction="row" spacing={2} alignSelf="center">
              <Button
                ...
                onClick={async () => {
                  setLoading(true);
                  if (inputMode === "1") {
                    const res = await extendedVigenereCipher(
                      inputText,
                      cipherkey,
                      true
                    );
